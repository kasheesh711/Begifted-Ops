...
import type { CalendarView, LinePreview, RiskFilter, SortField, SortState, Toast } from "@/lib/dashboard/ui-helpers";
import {
  actionStatusLabel,
...
export function DashboardShell({ sessionUser }: { sessionUser: AppSessionUser }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    fetch(`/api/actions/history?studentKey=${encodeURIComponent(key)}`)
      .then((res) => (res.ok ? res.json() : { entries: [] }))
      .then((data: { entries: ActionHistoryEntry[] }) => {
        if (cancelled) return;
        actionHistoryCache.current[key] = data.entries;
        setActionHistory(data.entries);
      })
      .catch(() => {
    ...
    <main className="dashboard-page">
      ...
      <header className="dashboard-header">
        ...
        <div className="header-meta">
          ...
          <a className="ghost-button" style={{ padding: "4px 10px", fontSize: "0.78rem" }} href="/api/auth/signout?callbackUrl=/signin">
            Sign out
          </a>
        </div>
      </header>
