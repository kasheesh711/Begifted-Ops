      ...
      <section className="panel" style={{ position: "sticky", top: 0, zIndex: 5, padding: "8px 10px" }}>
        ...
        <div style={{ marginTop: 6, fontSize: "0.78rem" }}>
          {student.actionState ? (
            ...
            <span className="muted">No action today</span>
          )}
        </div>
              ...
              actionHistory.map((entry, idx) => (
                <div className="history-entry" key={`${entry.updatedAt}-${idx}`} style={{ padding: "3px 0" }}>
                  ...
                  <div className="history-detail" style={{ fontSize: "0.78rem" }}>
                    {entry.actionType === "clear"
                      ? `Cleared by ${entry.updatedByName}`
                      : `${actionStatusLabel(entry.status as StudentActionStatus)} by ${entry.updatedByName}`}
                  </div>
                </div>
              ))
            ) : (
...
function renderProjectionBars(pkg: PackageRecord) {
  const maxBalance = Math.max(
    pkg.totalCredits || 1,
    ...pkg.projection.map((item) => Math.max(0, item.bal)),
  );

  return pkg.projection.slice(0, 8).map((item) => {
    const height = Math.max(18, (Math.max(0, item.bal) / maxBalance) * 110);
    const color =
      item.bal <= 0
        ? "var(--notify)"
        : item.bal < 2
    ...
    return (
      <div className="projection-bar-wrap" key={`${pkg.key}-${item.date}`}>
        <div className="projection-bar" style={{ height: `${height}px`, background: color }} />
        <div className="projection-label">{formatShortDate(item.date)}</div>
      </div>
    );
  });
}
