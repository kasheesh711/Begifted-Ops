...
status: executing
stopped_at: "Plan 03-05 SVC-04 route handler cutover complete (73698c5): 5 route handlers (/api/dashboard, /api/actions, /api/actions/bulk, /api/actions/history, /api/inactive) swapped from prod-snapshot Sheets-era code to the new Wisenet+Postgres service.ts (read path via getDashboardPayload + readHistory from db/queries) and the 'use server' actions.ts facade (write path via 5 mutation methods). Cache invalidation moves entirely into the facade per D-28 — the inline revalidateTag calls Plan 03-03 placed as a transition shim are removed. External JSON shapes preserved for the React client (one consumer-aligning rename: /api/actions/history returns { history } matching student-detail.tsx's actual consumer expectation; the Sheets-era { entries } key was wrong). actions-route.test.ts mocks @/lib/dashboard/actions facade (with vi.importActual spread to keep real normalizeStudentActionStatus); test count 3 → 4 with new clear-action assertion. Forced Rule 3 cascades: (1) drop `export const runtime = 'nodejs'` from all 5 routes per Plan 03-01 cacheComponents incompatibility (same cascade Plan 03-06 already documented for /api/health); (2) remove the action-helpers re-export in actions.ts that Plan 03-04 left in as a no-op — Plan 03-05's route-handler imports of the 5 async mutations made SWC report 'The module has no exports at all' because the stripped re-export apparently invalidates analysis of the whole 'use server' file. Build green, 170/170 tests (169 baseline + 1 new), tsc clean, lint green with empty allowlist. Next: Plan 03-07 TEST-03 + TEST-06 (cache-invalidation integration test gated on TEST_DATABASE_URL + chunked-transfer regression test); 03-08 SVC-05 lib/sheets deletion is now safe (zero route-handler imports remain)."
last_updated: "2026-04-30T03:46:30.000Z"
...
Plan: 8 of 11 (03-00 + 03-01 + 03-02 + 03-03 + 03-04 + 03-05 + 03-06 complete; next: 03-07 TEST-03/06)
...
| 3 | Service Cutover | 10 (SVC + TEST-04, TEST-06) | Executing — 7/11 plans complete (03-00 baseline reconciliation 2026-04-29: 7 routes restored + 97 Phase 1+2 source files + 72 .planning artifacts; 03-01 cacheComponents 2026-04-29: cdc2ebe one-line flag + forced compat — 6 route runtime drops + 3 auth-page Suspense wraps; 03-02 SVC-08 archive-link affordance 2026-04-30: 1fbd42a — ARCHIVE_ACTION_SHEET_URL constant + Student Detail header link; 03-03 SVC-02 service.ts rewrite 2026-04-30: 0ef5f73 — service.ts rewritten with 'use cache: remote' + cacheTag + explicit 3-arg cacheLife + Wisenet+Postgres composer + auto-reactivation + 1.8MB sanity warn + structured try/catch logging via D-37 logger.ts + 3 route-handler inline revalidateTag(_, "max") cascade + lint flipped (single-arg deprecated form forbidden; ALLOWLIST=()) + 4 D-35 baseline tsc fixes + db/client.ts Proxy lazy-init + p-limit serverExternalPackages + .gitignore *.tsbuildinfo; build green, 164/164 tests, tsc clean; 03-04 SVC-03 actions.ts use-server facade 2026-04-30: ff33a11 — 5 async mutations [setStudentAction, clearStudentAction, bulkSetAction, markInactiveStudent, clearInactiveStudent] writing-then-revalidateTag(tag, "max") per D-28; sync helpers split to co-located action-helpers.ts [Rule 3 — Next.js 16 SWC enforces async-only exports for 'use server' files at BUILD time, not just runtime; re-exports of sync functions also stripped]; 9 caller files updated to import from new module; markInactive schema deviation: facade accepts markedByName for API symmetry but only threads markedByEmail into DB; build green, 164/164 tests, tsc clean, lint green with empty allowlist; 03-05 SVC-04 route handler cutover 2026-04-30: 73698c5 — 5 route handlers (dashboard, actions, actions/bulk, actions/history, inactive) swapped to new service.ts + actions.ts facade; cache invalidation centralized in facade per D-28 (inline revalidateTag calls from 03-03 transition removed); actions-route.test.ts mocks @/lib/dashboard/actions facade with vi.importActual spread; test count 3 → 4 with new clear-action branch; consumer-aligning rename of /api/actions/history response key entries → history (Sheets-era key was wrong); 2 forced Rule 3 cascades — (a) drop `export const runtime = "nodejs"` from all 5 routes per Plan 03-01 (same cascade as 03-06), (b) remove action-helpers re-export in actions.ts that 03-04 left as a no-op — Plan 03-05's route-handler imports of the 5 async mutations made SWC report "The module has no exports at all" because the stripped re-export invalidates analysis of the whole 'use server' file; build green, 170/170 tests, tsc clean, lint green with empty allowlist; 03-06 SVC-07 /api/health structured probe per D-32 2026-04-30: 18a7728 — Sheets-era handler replaced with parallel probes for Wisenet/Postgres/Auth via Promise.all; D-32 rollup any-down -> down(503), any-degraded -> degraded(200), all-ok -> ok(200); new health-route.test.ts 5 vitest cases + T-03-06-1 info-disclosure assertion; forced cascade dropped `export const runtime = "nodejs"` per Plan 03-01 cacheComponents incompatibility; behavior note — wisenetFetch's internal 15s AbortSignal overrides 5s init.signal at runtime, queued tighten as deferred; build green, 169/169 tests [164 baseline + 5 new], tsc clean, lint green); TEST-04 deferred to Phase 4 per D-34; next 03-07 TEST-03 + TEST-06 then 03-08 lib/sheets deletion (now safe — zero route-handler imports remain) |
...
| /api/health structured probe per D-32 (03-06) | Replaced Sheets-era handler with `Promise.all([probeWisenet(), probePostgres(), probeAuth()])` plus D-32 rollup. probeWisenet hits cheap `/institutes/v3/{centerId}/students?page_size=1`. probePostgres runs `db.execute(sql\`SELECT 1\`)` via Drizzle HTTP. probeAuth checks `getAuthEnv()` presence only (no live token mint). Status code 503 only on overall "down"; 200 covers ok and degraded. No requireSessionUser (uptime monitors), no cache directive (live every call), no `export const runtime = "nodejs"` (Plan 03-01 cacheComponents cascade). deployedAt populated conditionally via `process.env.VERCEL_DEPLOYMENT_ID` spread. |
| wisenetFetch effective timeout note (03-06) | wisenetFetch (lib/wisenet/client.ts) currently overrides `init.signal` with its own internal `AbortSignal.timeout(15_000)` — the spread lays caller fields first then the explicit `signal:` overwrites. /api/health probeWisenet's call-site `AbortSignal.timeout(5_000)` is preserved per must_have key_link spec but discarded at runtime; effective probe bound is 15s. Acceptable for uptime polling cadence (typically ≥30s); a one-line conditional in client.ts (`signal: init.signal ?? AbortSignal.timeout(TIMEOUT_MS)`) would tighten this — queued as deferred follow-up, not blocking SVC-07 close. |
| T-03-06-1 info-disclosure mitigation locked in via test (03-06) | Per-subsystem `error` field captures `error.message` only (NOT `error.stack`). Auth error case: `getAuthEnv()` throws `Missing required environment variable: <NAME>` — env var NAME, not value. Asserted explicitly in health-route.test.ts: `expect(body.subsystems.auth.error).not.toContain("postgresql://")`. Wisenet error message format is `Wisenet ${status} at ${path}` per WisenetError.constructor — pathname only, no creds (REDACT_KEYS already strips PII keys from the body before the message is built). |
| 5 route handlers swapped to actions.ts facade — cache invalidation centralized per D-28 (03-05) | The 3 inline `revalidateTag(DASHBOARD_CACHE_TAG, "max")` calls Plan 03-03 placed as a transition shim in /api/actions, /api/actions/bulk, /api/inactive are removed. The single-source-of-truth for cache invalidation is now actions.ts facade (5 mutation methods, each writing-then-invalidating). Route handlers have ZERO `next/cache` imports. Test mocks updated: actions-route.test.ts mocks `@/lib/dashboard/actions` (not `@/lib/sheets/actions`) with `vi.importActual` spread to keep real `normalizeStudentActionStatus` available alongside mock 5 mutation methods. |
| Removed action-helpers re-export from actions.ts (03-05 Rule 3 cascade) | Plan 03-04 left `export { ... } from "@/lib/dashboard/action-helpers"` in actions.ts as a documented no-op (no production caller imported actions.ts at the time). Plan 03-05 wired 5 route handlers to import the 5 async mutations from actions.ts — at which point SWC reported "The module has no exports at all" against actions.ts. The stripped re-export apparently invalidates analysis of the whole 'use server' file, not just the re-export line. Removal of the re-export restored export visibility for the 5 async mutations. All sync-helper callers were already migrated to action-helpers.ts in Plan 03-04, so removal is downstream-safe. Lesson for future 'use server' file design: re-export statements are immediately verboten in 'use server' files, not "no-op-for-now". |
| /api/actions/history response key entries → history (03-05 consumer-aligning rename) | The Sheets-era prod-snapshot handler returned `{ entries: [...] }`. The React client (student-detail.tsx) reads `response.history`. The Sheets-era key was a regression; Plan 03-05's rename FIXES it. No client-side change required. Other compatibility-aligning shape simplifications: /api/actions returns `{ ok: true }` (was `{ studentKey, actionState }` — client only reads response.ok); /api/actions/bulk returns `{ updated: string[] }` (was `{ updated: ActionState[] }` — client consumes array length only); /api/inactive returns `{ ok: true }` (was `{ studentKey, inactive: bool }` — client reads response.ok only). All non-breaking. |
...
| Phase 03 P05 | 6min | 2 tasks (+2 Rule-3 cascades) | 7 files |
| Phase 03 P06 | 4min | 2 tasks (+1 Rule-3 cascade) | 2 files |
...
**Last session:** 2026-04-30T03:46:30.000Z
**Stopped at:** Plan 03-05 SVC-04 route handler cutover complete (73698c5): 5 route handlers (/api/dashboard, /api/actions, /api/actions/bulk, /api/actions/history, /api/inactive) swapped from prod-snapshot Sheets-era code to the new Wisenet+Postgres service.ts (read path) and the 'use server' actions.ts facade (write path). Cache invalidation moves entirely into the facade per D-28 — the inline revalidateTag calls Plan 03-03 placed as a transition shim are removed. External JSON shapes preserved for the React client (with one consumer-aligning rename: /api/actions/history returns { history } matching student-detail.tsx's actual consumer expectation). actions-route.test.ts mocks @/lib/dashboard/actions facade with vi.importActual spread; test count 3 → 4. Forced Rule 3 cascades: (1) drop `export const runtime = 'nodejs'` from all 5 routes per Plan 03-01 cacheComponents incompatibility (same cascade as 03-06); (2) remove the action-helpers re-export in actions.ts that Plan 03-04 left as a documented no-op — Plan 03-05's route-handler imports of the 5 async mutations triggered SWC's "The module has no exports at all" because the stripped re-export apparently invalidates analysis of the whole 'use server' file. Build green, 170/170 tests (169 baseline + 1 new), tsc clean, lint green with empty allowlist. Next: Plan 03-07 (TEST-03 cache-invalidation integration test gated on TEST_DATABASE_URL + TEST-06 chunked-transfer regression test); 03-08 lib/sheets deletion is now safe (zero route-handler imports remain).
...
**Executing Phase:** 03 (service-cutover) — Plan 03-00 complete 2026-04-29T15:46:25Z (3min, 3 sub-tasks, 176 files); Plan 03-01 complete 2026-04-29T15:56:45Z (5min, 1 task + 2 forced cascades, 10 files, commit cdc2ebe); Plan 03-02 complete 2026-04-30T02:46:00Z (2min, 2 tasks combined, 2 files, commit 1fbd42a); Plan 03-03 complete 2026-04-30T03:00:30Z (11min, 3 tasks + 8 deviations [3 Rule-3 + 4 Rule-1 + 1 Rule-2], 13 files, commit 0ef5f73); Plan 03-04 complete 2026-04-30T03:18:18Z (8min, 1 task + 1 architectural cascade [Rule-3 SWC async-only enforcement at build time + Rule-1 schema deviation], 11 files, commit ff33a11); Plan 03-05 complete 2026-04-30T03:46:30Z (6min, 2 tasks + 2 Rule-3 cascades [drop `export const runtime = 'nodejs'` per Plan 03-01 + remove action-helpers re-export in actions.ts that Plan 03-04 left as no-op — SWC strip invalidated whole 'use server' file when route handlers imported the 5 async mutations], 7 files, commit 73698c5); Plan 03-06 complete 2026-04-30T10:32:00Z (4min, 2 tasks + 1 Rule-3 cascade [drop `export const runtime = 'nodejs'` per Plan 03-01 cacheComponents incompatibility], 2 files, commit 18a7728)
